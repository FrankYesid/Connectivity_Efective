...
for s = SS
    ...
    for clas = 1:2
        ...
        for freq = 1:2
            ...
            Cx_1{clas}{freq} = squeeze(mean(mean(abs(Cx{clas}(:,:,f1:f2,67:71)),3),4));
            %             for v =1:size(Cx{clas},4)% [1,4,8,11,14,17,20,24,27,30,33,36,40,43]%1:size(Cx_all{s}{clas},4)
            Cx_{clas}{freq} = threshold_proportional(Cx_1{clas}{freq},um);
...
%                     tem__(logical(chan_),logical(chan_)) = 0;
    ...
    %             saveas(gca,['F:\Dropbox\[5] IOP DynamicGroupCx\figure\funcional_25_45\histogram\Cx2_s_' num2str(s) '_cl_' num2str(clas) '_fr_' num2str(freq) '_V_' num2str(v)],'png')
    %             close
    %         end
    %     end
...
% %             saveas(gca,['F:\Dropbox\[5] IOP DynamicGroupCx\figure\funcional_25_45\head_e\'...
% %                 'Cx2e_s_' num2str(s) '_cl_' num2str(clas) '_fr_' num2str(fr) '_V_65_71' ...%num2str(v)...
% %                 ],'epsc')
%             saveas(gca,['F:\Dropbox\[5] IOP DynamicGroupCx\figure\funcional_25_45\head_e\'...
%                 'Cx2c_s_' num2str(s) '_cl_' num2str(clas) '_fr_' num2str(fr) '_V_65_71' ...% num2str(v)...
%                 ],'png')
%             close
%             %             end
%             % %         if clas == 1
...
%     end
    ...
    %             subplot('position',[x1,y1,w,h]);
    %             imagesc(1:64,1:64,mean(Cx_1{clas}{freq}(:,:,65:71),3))
    ...
    %                 'Cx2_sin_thresholds_s_' num2str(s) '_cl_' num2str(clas) '_fr_' num2str(fr) '_V_65_71'...% num2str(v)...
    %                 ],'epsc')
    %             saveas(gca,['F:\Dropbox\[5] IOP DynamicGroupCx\figure\Imag_fun_ult\Cx_sin_umbral\'...
    %                 'Cx2_sin_thresholds_s_' num2str(s) '_cl_' num2str(clas) '_fr_' num2str(fr) '_V_65_71'...% num2str(v)...
    %                 ],'png')
    ...
    %             x1 = 0.06;     y1 = 0.06;     w = 0.92;      h = 0.92;
    %             subplot('position',[x1,y1,w,h]);
    %             imagesc(1:64,1:64,mean(Cx_{clas}{freq}(:,:,65:71),3))
    %             set(gca,'XTick',2:2:64,'XTickLabel',num2str([2:2:64]'),...
    %                 'YTick',2:2:64,'YTickLabel',num2str([2:2:64]'),...
    %                 'XTickLabelRotation',90,...
    %                 'TickLabelInterpreter','latex','FontSize',8)
    %             saveas(gca,['F:\Dropbox\[5] IOP DynamicGroupCx\figure\Imag_fun_ult\Cx_con_umbral\'...
    %                 'Cx2_con_thresholds_s_' num2str(s) '_cl_' num2str(clas) '_fr_' num2str(fr) '_V_65_71'...% num2str(v)...
    %                 ],'epsc')
    %             saveas(gca,['F:\Dropbox\[5] IOP DynamicGroupCx\figure\Imag_fun_ult\Cx_con_umbral\'...
    %                 'Cx2_con_thresholds_s_' num2str(s) '_cl_' num2str(clas) '_fr_' num2str(fr) '_V_65_71'...% num2str(v)...
    %                 ],'png')
    ...
    for clas = 1:2
        for freq = 1:2
            ...
            Cx_l{clas}{freq} = squeeze(mean(mean(abs(Cx{clas}(:,:,f1:f2,67:71)),3),4));
        end
    end
    v = 182;
...
%             poss = Cx_l2.*squareform(C)>0; %sum(poss)
            ...
            saveas(gca,['F:\Dropbox\[5] IOP DynamicGroupCx\figure'...
                '\funcional_25_45\head_linked_e\Cx3_s_' num2str(s) '_cl_' num2str(clas) '_fr_' num2str(freq) '_V_' num2str(v)],'png')
            close
        end
    end
end
