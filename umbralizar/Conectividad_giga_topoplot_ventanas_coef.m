...
% SS([29,34]) = [];
a = 1;
% load max_min_topo_zone_motor.mat
...
%             [Cx]=ordena_nuevo_cx(Cx_all);
            ...
            for v = 1:numel(vt)%v1:11:v2
                ...
                rel = clustering_coef_wd((Cx_1)./(ma__));%sum(squareform(Cx_1.*mk));
...
%                 %                 saveas(gca,['D:\Dropbox\ERD\Conectividad funcional\Conectividad Giga\Resultado_pru_gc\Cx_ventana\eps\Cx_s_',num2str(s),'_cl_',num2str(clas),'_fr_',num2str(freq),'_V_',num2str(v)],'eps')
                saveas(gca,['F:\Dropbox\[5] IOP DynamicGroupCx\figure\funcional_e\6Cx_sub_',num2str(s),'_cl_',num2str(clas),'_fr_',num2str(freq),'_V_',num2str(va(b))],'png')
                b = b+1;
