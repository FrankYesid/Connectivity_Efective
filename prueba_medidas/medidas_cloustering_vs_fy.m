...
for s = SS
    ...
    %     global_eff_= zeros(1,size(Cyx{1},4));
    local_eff_ = cell(size(Cx{1},4),1);%zeros(64,numel(Nfreq),size(Cx{1},4));
    ...
    for clas = 1:2
        ...
        for freq = Nfreq
            ...
            Cx_1{freq} = squeeze(mean(abs(cx_f(:,:,freq:freq+2,:)),3));
        end
        for freq = Nfreq
            for v =1:size(cx_f,4)
                %                 tic
                Cx_{freq}{v} = squeeze(Cx_1{freq}(:,:,v));%threshold_proportional(,um);
            end
        end
        parfor freq = Nfreq
            local_eff_{freq} = cellfun(@(x) efficiency_wei(x,1),Cx_{freq},'UniformOutput',false);
            %                 if strcmp(conect,'funcional')
            %                     if strcmp(Method,'degree')           % degree
            %                         %                             CCx_{v} = degrees_und(squeeze(Cx_));
            %                         %                         elseif strcmp(Method,'strengths')    % strengths
            ...
            %                         %                             transitivity_(:,freq,v) = transitivity_wu(Cx_);
            %                     elseif strcmp(Method,'local_eff')    % local efficiency
            %                 local_eff_{v}   = efficiency_wei(Cx_,1);
            %                     elseif strcmp(Method,'global_eff')   % global efficiency
            %                         global_eff_(:,v)   = efficiency_wei(Cx_,0);
            %                     elseif strcmp(Method,'path_len')     % path length
            %                         [path_len_(:,v),~,~,~,~] = charpath(distance_wei(weight_conversion(Cx_,'lengths')));
...
%             toc
    ...
    save(['D:\graph_s',num2str(s),'_5.mat'],'local_eff')
end% subjects